...
export function getPlanTagline(planId, billingCycle) {
  return `${formatLeadCountForBilling(planId, billingCycle)} · ${formatUserCount(planId)} · ${formatTemplateCount(planId)}`;
}
...
export function getPlanFeatures(planId, billingCycle) {
  const leadLine = formatLeadCountForBilling(planId, billingCycle);
